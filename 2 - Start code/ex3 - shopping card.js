...
function getCartTotalAmount() {
  ...
  SHOPPING_CART.forEach((p) => result += p.quantity*PRODUCTS.find(pf => pf.id === p.id).price)
  return result;
}
...
function addProductToCart(productId) {
  // Write your code here
   let product_from_store = PRODUCTS.find((ps) => ps.id === productId)
    if(product_from_store) {
      let add_product = SHOPPING_CART.find((pc) => pc.id === productId);
      if(add_product) {
...
function removeProductFromCart(productId) {
  ...
  let product_cart = SHOPPING_CART.find((pc) => pc.id == productId);
  let index = SHOPPING_CART.indexOf(product_cart)
  if(product_cart) {
    product_cart.quantity--;
    if(product_cart.quantity == 0) {
      SHOPPING_CART.splice(index, index);
    }
    console.log("Product remove from cart successful!");
  }
}
